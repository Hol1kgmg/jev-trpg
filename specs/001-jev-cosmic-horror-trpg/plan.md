...
待機画面で自分と敵を確認してからセッションを始め、いきなり怪異と対峙する。
各ターン、プレイヤーは行動の方向性を 4 択で宣言し、任意で詳細を自由入力する。
詳細を Jev（TypeSafe AI の System One モデル）が構造化値に解釈し、
成否の算出と状態更新はコード側の純関数が行う。最大8ターン、1プレイ8〜12分。

技術的な骨子は 3 つ。

1. **Jev は解釈のみ**。Vercel AI Gateway 経由で `experimental_evaluate` を 1 ターン 1 回だけ
   呼び、6 つの質問（choice 1 / score 2 / boolean 3）をまとめて投げる。行動種別はプレイヤーの
   4 択がそのまま入るので Jev には訊かない（FR-030）。成功率の算出・d100・状態更新は
...
3. **描写はテンプレート**。`(怪異の状態, 行動方向性, 結果段階)` をキーに複数バリエーションを持ち、
...
research.md R-009）。実装側の負担は、画像素材を `web/src/data/assets.ts` の
...
**Testing**: Vitest。`web/src/lib/jev/client.ts` を `vi.mock` で差し替え、d100 は `rng` 注入で決定化。
...
アプリはリポジトリ直下ではなく `web/` に置き、パッケージマネージャは pnpm
...
|---|---|---|
| I. 判定はコード、解釈だけが LLM | Jev の応答は `Judgment` に正規化されるだけで状態を変えない。成功率の式・d100・HP/正気度/手がかり/ターンの更新は `web/src/lib/game/` の純関数（research.md R-006）。描写は `web/src/data/templates/` から選択 | PASS / PASS |
| II. 1ターン1回の Jev 呼び出し | `POST /api/turn` のハンドラ内で `judge()` を 1 回だけ呼ぶ。6 問を 1 リクエストにまとめる。`state` は判定に必要な最小限（contracts/jev-questions.md）。`providerMetadata.typesafe.confidence.skill` が 0.5 未満（欠損含む）なら `ambiguous`。呼び出し失敗は再試行せずフォールバックで完結 | PASS / PASS |
| III. 生成物は解けることを保証する | 生成直後に `clearCondition.requiredClueIds` が観察で入手可能な `Clue` の id 全体に含まれることを集合演算で検証し、失敗なら破棄して再生成（上限 50 回）。100 シードのテストで担保（research.md R-007 / SC-003） | PASS / PASS |
...
- 画像素材は `web/src/data/assets.ts` の `AssetEntry[]` に登録したもののみ参照する。
...
├── app/
│   ├── layout.tsx
│   ├── page.tsx                 # 待機画面とプレイ画面（状態で切り替え・エンディング）
...
│   │   ├── generate.ts          # 探索者・怪異・クリア条件・手がかりの生成と解ける保証
...
│   ├── jev/
│   │   ├── questions.ts         # 6問の定義（contracts/jev-questions.md）
...
└── data/
    ...
    └── templates/
        ├── scenes.ts            # 怪異の状態ごとの場面描写
        ├── outcomes.ts          # (stageKey, direction, outcome) → 文字列配列
        ├── clues.ts             # 手がかり文
...
`web/src/lib/game/` の純関数を両者が直接 import できるほうが型の往復が減るため。
