...
- 入力欄からフォーカスが外れて `previewDebounceMs`（3 秒）経ったら自動で `/api/preview` を呼び、Jev を 1 回走らせる。入力中は測らない
...
- 実行後のログにも同じ内訳を残す
- 詳細が空欄の行動は Jev を呼ばず、妥当性の補正 0・決着なしで確定する（`emptyJudgment`）。事前判定も送らず、画面は測定後と同じ表示にする
